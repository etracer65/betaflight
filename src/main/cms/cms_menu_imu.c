...
static const void *cmsx_cancelRatesReset(displayPort_t *pDisp, const void *self);
...
static const OSD_Entry cmsx_menuRateTypeChangeWarningEntries[] =
{
    ...
    { "", OME_Label, NULL, NULL, 0},
    { "BACK", OME_Funcall, cmsx_cancelRatesReset, NULL, 0 },
...
static OSD_Entry cmsx_menuRateProfileEntries[] =
{
    ...
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rcRates[FD_ROLL],  0, 0, 0, 0 }, 0 },
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rates[FD_ROLL],    0, 0, 0, 0 }, 0 },
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rcExpo[FD_ROLL],   0, 0, 0, 0 }, 0 },

    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rcRates[FD_PITCH], 0, 0, 0, 0 }, 0 },
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rates[FD_PITCH],   0, 0, 0, 0 }, 0 },
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rcExpo[FD_PITCH],  0, 0, 0, 0 }, 0 },

    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rcRates[FD_YAW],   0, 0, 0, 0 }, 0 },
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rates[FD_YAW],     0, 0, 0, 0 }, 0 },
    { "",              OME_UINT8_SCALED, NULL, &(OSD_UINT8_SCALED_t) { &rateProfile.rcExpo[FD_YAW],    0, 0, 0, 0 }, 0 },
...
static void rateProfileMenuBuild(void)
{
    ...
    for (unsigned axis = 0; axis <= FD_YAW; axis++) {
        for (unsigned rateParamType = 0; rateParamType < 3; rateParamType++) {
            ...
            const int menuIndex = axis * 3 + rateParamType + 2;  // index position in the cmsx_menuRateProfileEntries array
...
static const void *cmsx_cancelRatesReset(displayPort_t *pDisp, const void *self)
{
    UNUSED(self);
    UNUSED(pDisp);
    rateProfile.rates_type = previousRatesType;
